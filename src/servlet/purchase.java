...
public class purchase extends HttpServlet {
 ...
 public void doPost(HttpServletRequest request, HttpServletResponse response)
  ...
  out.println("<title>Purchase for Customer</title></head><body>");
  out.println("<center>");
  ...
  out.println("<form method='POST' + action='purchase.html'>");
  out.println("<table border='0'>"+
	"<tr><td>ID</td><td bgcolor='red' ><input type='text' size='20' name='ID'" +
	"</td></tr>" +
	"<tr><td>Amount</td>" +
  ...
  out.println("<input type='submit' value='Purchase!!!'></td></form>");
  out.println("</td></tr><tr><td><a href='customer.html'>Back</td></tr></table>");
  if(request.getParameter("ID")!=null)
	{
	  Calendar date =new GregorianCalendar();
	  String orderdate,month,day;
	  ...
	  orderdate=date.get(Calendar.YEAR)+"-"+month+"-"+day;
	  String productID=request.getParameter("ID");
	  int customerID=IDget(request,response,"customer");
	  int purchaseID=IDgenerator(request,response,"purchase");
	  int productamount=Integer.parseInt(request.getParameter("amount"));
	  
	String changeProduct="UPDATE product_info SET amount=amount-"+productamount+" WHERE product_ID="+productID;
	String changeRecord="INSERT purchase_record(purchase_ID,customer_ID,product_ID,amount,order_date) VALUES("+purchaseID+","+customerID+","+productID+","+productamount+",'"+orderdate+"')";
	String ShowRecord="SELECT * FROM purchase_record WHERE purchase_ID="+purchaseID;
	InsertUserInfo(request,response,changeProduct);
	InsertUserInfo(request,response,changeRecord);
	response.reset();
     response.setContentType("text/html");
     updateUser(request,response,ShowRecord);
     // Here we use a PrintWriter to send text data
     // to the client who has requested the servlet
     PrintWriter out1 = response.getWriter();
     out1.println("<p>Thanks for purchase!!!</p><p>We will send you the product as soon as possible!</p><p><a href='customer.html'>Back</p>"+","+ID+","+purchaseID+","+orderdate+","+customerID+"");
	}
